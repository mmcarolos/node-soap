...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    ...
    this.element.$lookupType = nsName.name;
  ...
  if (Array.isArray(obj)) {
    for (var i = 0, item; item = obj[i]; i++) {
      var arrayAttr = self.processAttributes(item),
        correctOuterNamespace = (parentNamespace === '' ? '' : (parentNamespace || ns)); //using the parent namespace if given
        ...
        if (self.definitions.schemas) {
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              if(childParameterTypeObject.$baseNameSpace && childParameterTypeObject.$baseNameSpace!==namespace) { //this element has a base with another namespace (the correct one)
